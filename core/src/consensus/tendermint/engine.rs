...
fn aggregate_work_info(
    ...
    // let mut current_validators = validators.current_addresses(&header.hash());
    ...
    while start_of_the_current_term != header.number() {
        ...
        ctrace!(TEST_SCRIPT, "AGGREGATE: {}", header.number());
        current_validators = validators.current_addresses(&header.hash());
